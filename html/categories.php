<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Категории</title>
    <link rel="shortcut icon" href="./../img/src/logo.png" type="image/x-icon" />
    <link rel="stylesheet" href="./../swiper-bundle.min.css">
    <link rel="stylesheet" href="./../style.css"/>
  </head>
  <body>
  <?php
  include("header.php");
  ?>
    <section class="inner-first-screen">

        <img src="./../img/src/fonGL.jpg" alt="fon"class="main-image" >

        <div class="inner-first-screen_wrapper">
            <ul class="inner-first-screen_breadcrumbs">
                <li class="inner-first-screen_breadcrumbs_item">Главная</li>
                <li class="inner-first-screen_breadcrumbs_item">Категории</li>
            </ul>
            <h1 class="inner-first-screen_heading">КАТЕГОРИИ МАРШРУТОВ</h1>
            <p class="inner-first-screen_lead">Выбирай любую категорию маршрутов, сохраняй себе, чтобы не забыть пройти именно его и наслаждайся видами и историей мимо проходящих мест! </p>
        </div>
        
        
    </section> 

    <section class="marshruts">
         
        <div class="container-marshruts">
            <h2 class="visually-hidden">Категории</h2>
            <div class="marshruts_item">

                <div class="row">
                    <div class="col-6">
                        <a href="./types1.html" class="mar-head">
                            <h3 class="mar-heading">“По центральным улицам”</h3>
                        </a>
                        <p class="lead">Маршрут "По центральным улицам Питера" включает в себя популярные улицы и достопримечательности города. Весь маршрут пропитан историей и уникальной атмосферой!</p>
                        <ul class="marshruts-list">
                            <li class="marshruts-list_item">“Завораживающий Невский”</li>
                            <li class="marshruts-list_item">“Самый необычный маршрут”</li>
                            <li class="marshruts-list_item">“Петербург за 1 день”</li>
                            <li class="marshruts-list_item">“От Русского музея...”</li>
                            <li class="marshruts-list_item">“Сокровища Санкт-Петербурга: от Исаакиевского собора до Невского проспекта”</li>
                            <li class="marshruts-list_item">“Маршрут по Крюкову каналу””</li>
                        </ul>
                        <ul class="marshruts-info">
                            <li class="marshruts-info_item">6 маршрутов</li>
                            <li class="marshruts-info_item">от 1 до 5 км</li>
                        </ul>
                    </div>
                    <div class="col-4">
                        <img class="col-4_img" src="./../img/src/Marshruts/mar1.jpg" alt="photo1">
                     </div>
                </div>     
            </div>


            <div class="marshruts_item">
                <div class="row">
                    <div class="col-6">
                        <img class="col-4_img" src="./../img/src/Marshruts/mar2.jpg" alt="photo2">
                    </div>

                    <div class="col-4"> 
                        
                        <a href="./types2.html" class="mar-head">
                            <h3 class="mar-heading">“По Васильевскому острову”</h3>
                        </a>
                        <p class="lead">Маршрут "По центральным улицам Питера" включает в себя популярные улицы и достопримечательности города. Весь маршрут пропитан историей и уникальной атмосферой!.</p>
                        <ul class="marshruts-list">
                            <li class="marshruts-list_item">“Стрелка Васильевского сотрова”</li>
                            <li class="marshruts-list_item">“Увлекательный остров”</li>
                            <li class="marshruts-list_item">“Пешком по Ваське”</li>
                            <li class="marshruts-list_item">“Маленький и мистический островок”</li>
                            <li class="marshruts-list_item">“Весь остров за 1 день”</li>
                            <li class="marshruts-list_item">“Васильевский”</li>
                        </ul>
                        <ul class="marshruts-info">
                            <li class="marshruts-info_item">6 маршрутов</li>
                            <li class="marshruts-info_item">от 5 до 11 км</li>
                        </ul>
                    </div>
                </div>
                </div>
                    


            <div class="marshruts_item">
                <div class="row">
                    <div class="col-6">
                        <a href="./../html/types3.html" class="mar-head">
                            <h3 class="mar-heading">“По Петроградскому району”</h3>
                        </a>
                        <p class="lead">Маршрут "По центральным улицам Питера" включает в себя популярные улицы и достопримечательности города. Весь маршрут пропитан историей и уникальной атмосферой!</p>
                        <ul class="marshruts-list">
                            <li class="marshruts-list_item">“Камчатка Виктора Цоя”</li>
                            <li class="marshruts-list_item">“Завораживающая Петроградка”</li>
                            <li class="marshruts-list_item">“Доходный остров”</li>
                            <li class="marshruts-list_item">“Маршрут от Горьковской до Петроградской ”</li>
                            <li class="marshruts-list_item">“Гуляем по Петроградской стороне”</li>
                            <li class="marshruts-list_item">“Соврменное искусство на улицах Питера”</li>
                        </ul>
                        <ul class="marshruts-info">
                            <li class="marshruts-info_item">6 маршрутов</li>
                            <li class="marshruts-info_item">от 5 до 15 км</li>
                        </ul>
                    </div>
                    
                    <div class="col-4">
                        <img class="col-4_img" src="./../img/src/Marshruts/mar3.jpg" alt="photo3">
                    </div>
                </div>

        </div>
        
    </section>










    <div class="container_footer">
      <footer class="main-footer">
          <div class="logo-footer">
            <img src="./../img/src/logo-footer.svg" alt="logo" class="logo__img" />
          </div>
        <ul class="main-menu">
          <li class="main-menu__item">
            <a href="./categories.html" class="main-menu__link">КАТЕГОРИИ</a>
          </li>
          <li class="main-menu__item">
            <a href="./marshruts.html" class="main-menu__link">МАРШРУТЫ</a>
          </li>
          <li class="main-menu__item">
            <a href="./../index2.html" class="main-menu__link">ГЛАВНАЯ</a>
          </li>
          <li class="main-menu__item">
            <a href="./myMarshruts.html" class="main-menu__link">МОИ МАРШРУТЫ</a>
          </li>
          <li class="main-menu__item">
            <a href="#" class="main-menu__link">ПРОФИЛЬ</a>
          </li>
        </ul>
    </footer>
  </div>
    <div class="main-footer_last-line">
          <p class="main-footer_pharagraph">
            (С) Машруты по Санкт-Петербургу 2024. Все права защищены
          </p>
          <div class="soc-links-footer">
              <ul class="soc-links">
              <li class="soc-links__item">
                <a href="#" class="soc-links__link">
                  <svg viewBox="0 0 20 20" width="20" height="20"  fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                    <rect width="20" height="20" fill="url(#pattern0_46_406)"/>
                    <defs>
                    <pattern id="pattern0_46_406" patternContentUnits="objectBoundingBox" width="1" height="1">
                    <use xlink:href="#image0_46_406" transform="scale(0.0078125)"/>
                    </pattern>
                    <image id="image0_46_406" width="128" height="128" xlink:href="data:image/png;base64,"/>
                    </defs>
                    </svg>
                </a>
              </li>
              <li class="soc-links__item">
                <a href="#" class="soc-links__link">
                  <svg viewBox="0 0 20 20" 
                    width="20" height="20"  
                    fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                    <rect width="20" height="20" fill="url(#pattern0_46_405)"/>
                    <defs>
                    <pattern id="pattern0_46_405" patternContentUnits="objectBoundingBox" width="1" height="1">
                    <use xlink:href="#image0_46_405" transform="scale(0.0078125)"/>
                    </pattern>
                    <image id="image0_46_405" width="128" height="128" xlink:href="data:image/png;base64,"/>
                    </defs>
                    </svg>
                    
                </a>
              </li>
              <li class="soc-links__item">
                <a href="#" class="soc-links__link">
                  <svg viewBox="0 0 20 20" width="20" height="20"  fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                    <rect width="20" height="20" fill="url(#pattern0_46_404)"/>
                    <defs>
                    <pattern id="pattern0_46_404" patternContentUnits="objectBoundingBox" width="1" height="1">
                    <use xlink:href="#image0_46_404" transform="scale(0.0078125)"/>
                    </pattern>
                    <image id="image0_46_404" width="128" height="128" xlink:href="data:image/png;base64,"/>
                    </defs>
                    </svg>
                    </defs>
                  </svg>
                </a>
              </li>
            </ul>
          </div>
          
    </div>


  <div class="modal-form_wrapper jsVisually-hidden">
    <section class="modal-form">
      <button aria-label="Закрыть" class="jsModalClose modal-form_close">Закрыть</button>
      <p class="modal-form_heading">Создавайте свои собственные маршруты, добавляйте любимые места и делитесь ими с другими!</p>
      <form action="send.php" class="form-modal-form">

        <div class="form-inner_wrapper">
          <label for="name" class="form_label">
            Название маршрута*
            <input name="name" id="name" type="text" class="form_input">
          </label>
          <label for="width" class="form_label">
            Протяжённость маршрута*
            <input name="width" id="width" type="text" class="form_input">
        </label>
        </div>
        <label for="text" class="form_label">
          Описание*
          <textarea name="opisanie" id="text" cols="30" rows="5" class="form_textarea"></textarea>
        </label>

        <div class="form-button_wrapper">
          <button class="button">Создать маршрут</button>
        </div>
        
      </form>
    </section>
  </div>
  


    <script src="./../js/main.js"></script>
    <script src="./../js/swiper-bundle.min.js"></script>

  </script>
  </body>
</html>
